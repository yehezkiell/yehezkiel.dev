...
PDP Page has several restriction cases that will block the Button. It will show the particular view on top of the button to prevent users click the button. Each restriction have different purposes but still we got the data from a struct in P2Data.
...
<com.tokopedia.sortfilter.SortFilter
        app:sort_filter_relationship="[AND|OR]"
        app:sort_filter_type="[quick|advanced]" />
```
